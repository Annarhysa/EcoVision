...
function App() {
  ...
  // Fetch deforestation data from the backend
useEffect(() => {
  axios.get('http://127.0.0.1:5000/deforestation-data')
    .then(response => {
  ...
  return (
    <div>
      ...
      {climateChangeData && (
        <div>
          <p>Average Temperature: {climateChangeData.average_temperature}</p>
          <p>Co2 Concentration: {climateChangeData.co2_concentration}</p>
